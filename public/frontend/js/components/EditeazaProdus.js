const EditeazaProdus = {
    name: 'EditeazaProdus',
    emits: ['arata-produse'],
    template: 
    `
    <!-- Modal pentru editarea produselor -->
    <div class="modal fade" id="editeazaProdusModal" tabindex="-1" aria-labelledby="editeazaProdustModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editeazaProdusModalLabel">Editeaza produsul</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form @submit.prevent="actualizeazaProdus()">
                        <div class="mb-3">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" @click="actualizeazaProdus()">
    ...
    setup(props, { emit }) {
        ...
        const actualizeazaProdus = () => {
            axios.post(props.updatelink + '?id=' + props.product.id, props.product)
            .then(response => {
                console.log('Product modified:', response.data);
                
                // Închide modalul
                const modal = bootstrap.Modal.getInstance(document.getElementById('editeazaProdusModal'));
                if (modal) {
                    modal.hide();
                }
